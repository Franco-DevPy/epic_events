        ...
        if choice == "Create contract":
                        ...
                        for c in contracts:
                            print(
                                ...
                                f"[ID {
                                    c.id}] Client: {
                                    c.client.full_name} | ({
                                    c.client.company_name}) | total: {
                                    c.total_amount}€ | remaining: {
                                    c.remaining_amount}€ | {
                                    c.status.value}" +
                                Style.RESET_ALL)
            ...
            contract_choices = [
                f"{
                    ...
                    c.total_amount}€ | remaining: {
                        c.remaining_amount}€ | {
                            c.status.value}" for c in contracts]
            ...
            total_amount = questionary.text(
                "New total amount (leave empty):").ask()
            remaining_amount = questionary.text(
                "New remaining amount (leave empty):").ask()
            status_choice = questionary.select(
                "New status:",
            ...
            remaining_amount = float(
                remaining_amount) if remaining_amount else None
            status = EnumStatus(
                status_choice) if status_choice != "skip" else None
            update_contract_service(
