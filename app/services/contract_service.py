...
def create_contract_service(
    ...
    if not client_id or total_amount is None:
        print(
            Fore.RED +
            "Error: Client and total amount are required." +
            Style.RESET_ALL)
        return None
    try:
        if current_user.role not in [
                EnumRole.commercial.value,
                EnumRole.management.value]:
            print(
                Fore.RED +
                "Permission denied: Only commercial or management users can create contracts." +
                Style.RESET_ALL)
            return None
        ...
        if client.commercial_id != current_user.id:
            print(
                Fore.RED +
                "Permission denied: This client is not yours." +
                Style.RESET_ALL)
            return None
...
def get_all_contracts_service(token):
    ...
    try:
        ...
        if current_user.role == EnumRole.management.value:
            ...
            filtered = [
                c for c in contracts if c.commercial_id == current_user.id]
            if not filtered:
            ...
            print(
                Fore.YELLOW +
                "Support role has no access to contracts." +
                Style.RESET_ALL)
            return []
        return []
...
def update_contract_service(
    ...
    try:
        ...
        if current_user.role == EnumRole.commercial.value and contract.commercial_id != current_user.id:
            print(
                Fore.RED +
                "Permission denied: You can only update your own contracts." +
                Style.RESET_ALL)
            return None
...
def delete_contract_service(contract_id, token):
    ...
    try:
        ...
        if current_user.role != EnumRole.management.value:
            print(
                Fore.RED +
                "Permission denied: Only management can delete contracts." +
                Style.RESET_ALL)
            return False
...
def get_unsigned_contracts_service(token):
    ...
    try:
        ...
        print(
            Fore.RED +
            f"Error retrieving unsigned contracts: {e}" +
            Style.RESET_ALL)
        return []
...
def get_unpaid_contracts_service(token):
    ...
    try:
        ...
        print(
            Fore.RED +
            f"Error retrieving unpaid contracts: {e}" +
            Style.RESET_ALL)
        return []
