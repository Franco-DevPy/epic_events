    ...
    try:
        ...
        if verify_password(password, user.password_hash):
            print(
                Fore.GREEN +
                f"Welcome {
                    user.full_name}! You are logged in as {
                    user.role}." +
                Style.RESET_ALL)
            return user
        else:
...
def create_user_service(full_name, email, password, role, token):
    ...
    if current_user.role != EnumRole.management.value:
        print(
            Fore.RED +
            "Permission denied: Only management can create users." +
            Style.RESET_ALL)
        return None
...
def get_all_users_service(token):
    ...
    if current_user.role != EnumRole.management.value:
        print(
            Fore.RED +
            "Permission denied: Only management can view all users." +
            Style.RESET_ALL)
        return []
...
def update_user_service(user_id, full_name, email, password, role, token):
    ...
    if current_user.role != EnumRole.management.value:
        print(
            Fore.RED +
            "Permission denied: Only management can update users." +
            Style.RESET_ALL)
        return None
...
def delete_user_service(user_id, token):
    ...
    if current_user.role != EnumRole.management.value:
        print(
            Fore.RED +
            "Permission denied: Only management can delete users." +
            Style.RESET_ALL)
        return False
